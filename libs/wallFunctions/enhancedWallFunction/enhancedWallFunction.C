#include "enhancedWallFunction.H"

//////////////// constructor ///////////////////////

enhancedWallFunction::enhancedWallFunction (const char *path, double **U, double **V): wallFunction(path, U, V)
{print_wallFunction_information();}

	
//////////// public functions /////////////////////////////////////////////////////////////////////////

void enhancedWallFunction::print_wallFunction_information()
{
...
void enhancedWallFunction::calculate_wallsKinematicViscosity()
{
	int i,j; 
	double yPlus, gamma, uPlus_lam, uPlus_turb;
	...
	for(i=0;i<numberOfNodes_xDirection;i++)
	{
		...
		gamma = -0.01*yPlus*yPlus*yPlus*yPlus/(1.0 + 5*yPlus);
		uPlus_lam = yPlus;
		uPlus_turb = 1.0/kappa *log(E*yPlus);

		nu_bottomWall[i] = nu[0][i]*(yPlus/(exp(gamma)*uPlus_lam + exp(1.0/gamma)*uPlus_turb));
	}
	...
	for(j=0;j<numberOfNodes_yDirection;j++)
	{
		...
		nu_leftWall[j] = nu[j][0]*(yPlus/(exp(gamma)*uPlus_lam + exp(1.0/gamma)*uPlus_turb));
	}
	...
	for(j=0;j<numberOfNodes_yDirection;j++)
	{
		...
		nu_rightWall[j] = nu[j][numberOfNodes_xDirection-1]*(yPlus/(exp(gamma)*uPlus_lam + exp(1.0/gamma)*uPlus_turb));
	}
...
void enhancedWallFunction::write_solutionField()
{
	int i,j;
